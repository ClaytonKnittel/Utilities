 ...
 * 
 * <pre>
 *   List list = Collections.synchronizedList(new LList(...));
 * </pre>
 *
 * <p>
...
public class LList<E> extends AbstractSequentialList<E> implements List<E>, Deque<E>, Cloneable, java.io.Serializable {
	 ...
	 */
	public LList(Collection<? extends E> c) {
		this();
		addAll(c);
	}
	...
	private class DelListItr implements ListIterator<E>, Iterable<E> {
		private Node<E> lastReturned;
		private Node<E> next;
		private int nextIndex;
		private int expectedModCount = modCount;
		private Delete<E> del;

		DelListItr(int index, Delete<E> shouldDelete) {
			// assert isPositionIndex(index);
			next = (index == size) ? null : node(index);
			nextIndex = index;
			this.del = shouldDelete;
		}

		public boolean hasNext() {
			if (nextIndex >= size)
				return false;
			while (del.delete(next.item)) {
				next();
				remove();
				if (next == null)
					return false;
	...
	public static interface Delete<E> {
		boolean delete(E e);
	}
	...
	public static class Node<E> {
		...
		public String toString() {
			...
			return prev.item + " -> " + item + " -> " + next.item;
		}
	}
	...
	@SuppressWarnings("unchecked")
	private LList<E> superClone() {
		try {
			return (LList<E>) super.clone();
	...
	/**
	 * Returns a shallow copy of this {@code LList}. (The elements themselves are
	 * not cloned.)
	 *
	 * @return a shallow copy of this {@code LList} instance
	 */
	public Object clone() {
		LList<E> clone = superClone();
	...
	public void insertionSort(Comparator<E> c) {
		...
		Node<E> n = first;
		Node<E> p;
		loop: while (n.next != null) {
			n = n.next;
			
			E e = n.item;
			p = n.prev;
			while (p != null) {
				if (c.compare(e, p.item) >= 0) {
					n = insertAfter(n, p);
					continue loop;
				}
				p = p.prev;
	...
	private Node<E> insertAfter(Node<E> a, Node<E> node) {
		if (node.next == a)
			return a;
		Node<E> ret = a.prev;
		a.item = this.unlink(a);
		a.next = node.next;
		a.prev = node;
		node.next.prev = a;
		node.next = a;
	...
	private Node<E> insertFirst(Node<E> a) {
		Node<E> ret = a.prev;
		a.item = unlink(a);
		a.next = first;
		a.prev = null;
	...
	/**
	 * Saves the state of this {@code LList} instance to a stream (that is,
	 * serializes it).
	 *
	...
	/**
	 * Reconstitutes this {@code LList} instance from a stream (that is,
	 * deserializes it).
	 */
	@SuppressWarnings("unchecked")
	...
	static final class LLSpliterator<E> implements Spliterator<E> {
		...
		final LList<E> list; // null OK unless traversed
		Node<E> current; // current node; null until initialized
		int est; // size estimate; -1 until first needed
		int expectedModCount; // initialized when est set
		int batch;// batch size for splits

		LLSpliterator(LList<E> list, int est, int expectedModCount) {
			this.list = list;
			this.est = est;
